...
@section('body')
    ...
    <hr />
    <form action="{{ route('professeurDash.update', $professeur->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col mb-3">
        ...
        <div class="row">
            <div class="col mb-3">
                ...
                <input type="tel" name="tel" class="form-control @error('tel') is-invalid @enderror" placeholder="Numero de téléphone" value="0{{ $professeur->tel }}" >
                @error('tel')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
