...
@section('body')
    <h1 class="mb-0">Ajouter un emploi</h1>
    <hr />
    <form action="{{ route('emploi.store') }}" method="POST">
        @csrf
        <div class="row mb-3">
            <div class="col">
                <input type="datetime-local" name="date" class="form-control @error('datePiement') is-invalid @enderror" placeholder="Date paiement" value="{{old('datePaiement')}}"> 
                @error('dataPaiement')
