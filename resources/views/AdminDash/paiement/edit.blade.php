...
@section('body')
    ...
    <hr />
    <form action="/paiementUpdate/{{$id}}" method="POST">
        @csrf
            ...
            <div class="form-group">
                ...
                <select name="formation_id" class="form-control @error('formation_id') is-invalid @enderror">
                    ...
                    @foreach ($formations as $formation)
                        <option value="{{ $formation->id }}" @if ($formation->id == $selectedFormationId) selected @endif>{{ $formation->specialite }} &nbsp;&nbsp; {{ $formation->periode }} &nbsp;&nbsp; {{ $formation->prix }}</option>
                    @endforeach
                </select>
                @error('formation_id')
